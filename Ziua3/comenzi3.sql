...
;
SELECT AVG (medie_per_student) FROM  (
	SELECT AVG(valoare) FROM note
	GROUP BY studenti_idstudent) medie_per_student;


SELECT studenti.nume, studenti.prenume, AVG(valoare) FROM 
	studenti JOIN note
    ON studenti.idstudent = note.studenti_idstudent
	GROUP BY studenti_idstudent
    HAVING AVG(valoare) > (SELECT AVG (medie_per_student) FROM  (
	SELECT AVG(valoare) FROM note
	GROUP BY studenti_idstudent) medie_per_student);
...
CREATE VIEW anonime AS
 SELECT CONCAT( LEFT(nume, 1), "*** " , SUBSTR(prenume, 1, 1)) , AVG(valoare) as nota FROM studenti 
	JOIN note ON studenti.idstudent = note.studenti_idstudent
	GROUP BY studenti.idstudent;

CREATE VIEW nume_stud AS
 SELECT nume FROM studenti;

SELECT * FROM nume_stud;

INSERT INTO nume_stud VALUES ("Nume test");
...
SELECT * FROM anonime;
INSERT INTO anonime SET nota = 2;
